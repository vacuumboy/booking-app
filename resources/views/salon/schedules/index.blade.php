<x-app-layout>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 animate-in fade-in duration-1000">
        ...
        <div class="lg:hidden bg-white shadow-sm border-b border-gray-100 animate-in slide-in-from-top duration-500">
            <div class="px-4 py-3 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-lg font-semibold text-gray-900 animate-in slide-in-from-left duration-700">{{ $selectedMaster->name }}</h1>
                </div>
                <a href="{{ route('salon.schedules.create', ['master_id' => $selectedMaster->id]) }}" 
                    ...
                    <div class="flex items-center space-x-4 animate-in slide-in-from-left duration-700">
                        <a href="{{ route('salon.masters.index') }}" 
                           class="flex items-center space-x-2 text-gray-600 hover:text-gray-900 transition-colors duration-200 group">
                            ...
                            <span class="font-medium">К мастерам</span>
                        </a>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-6" style="padding: 1%;">
            ...
            @if(session('success'))
                <div class="mb-6 bg-green-100 border-l-4 border-green-500 text-green-700 px-4 py-3 rounded-lg animate-in slide-in-from-left duration-500">
                    {{ session('success') }}
                </div>
            @endif

            @if(session('warning'))
                <div class="mb-6 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 px-4 py-3 rounded-lg animate-in slide-in-from-left duration-500">
                    {{ session('warning') }}
                </div>
            @endif

            @if(session('error'))
                <div class="mb-6 bg-red-100 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded-lg animate-in slide-in-from-left duration-500">
                    {{ session('error') }}
                </div>
            @endif

            @if($errors->any())
                <div class="mb-6 bg-red-100 border-l-4 border-red-400 text-red-700 px-4 py-3 rounded-lg animate-in slide-in-from-left duration-500">
                    <ul>
            ...
            <!-- Masters Selection -->
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-6 animate-in fade-in slide-in-from-bottom duration-700 delay-200">
                <div class="bg-gradient-to-r from-purple-500 to-pink-600 px-6 py-4">
                    <h2 class="text-xl font-bold text-white animate-in zoom-in duration-500 delay-300">👥 Выберите мастера</h2>
                </div>
                <div class="p-6">
                    <div class="flex flex-wrap gap-3">
                        @foreach($masters as $index => $master)
                            <a href="{{ route('salon.schedules.index', ['master_id' => $master->id]) }}" 
                               class="group px-4 py-3 rounded-xl transition-all duration-300 transform hover:scale-105 animate-in slide-in-from-bottom duration-500 {{ $selectedMaster->id == $master->id 
                                   ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg' 
                                   : 'bg-gray-100 hover:bg-gray-200 text-gray-800 hover:shadow-lg' }}"
                               style="animation-delay: {{ $index * 100 }}ms">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 rounded-full {{ $selectedMaster->id == $master->id ? 'bg-white bg-opacity-20' : 'bg-gray-300' }} flex items-center justify-center overflow-hidden transition-all duration-200">
                                        @if($master->photo_path)
                                            <img src="{{ asset('storage/' . $master->photo_path) }}" 
                                                 alt="{{ $master->name }}" 
                                                 class="w-full h-full object-cover">
                                        @else
                                            <span class="font-semibold {{ $selectedMaster->id == $master->id ? 'text-white' : 'text-gray-700' }}">
                                    ...
                                    @if($selectedMaster->id == $master->id)
                                        <svg class="w-5 h-5 text-white animate-in zoom-in duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Calendar -->
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-bottom duration-1000 delay-400">
                <!-- Calendar Header -->
                <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-blue-100 text-sm animate-in fade-in duration-500 delay-700">{{ $selectedMaster->name }}</p>
                        </div>
                                        ...
                                        @if($day['has_schedule'])
                                            <div class="space-y-1">
                                                ...
                                                <div class="flex space-x-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                    <a href="{{ route('salon.schedules.edit', ['schedule' => $day['schedule']->id, 'master_id' => $selectedMaster->id]) }}" 
                                                       class="bg-blue-500 hover:bg-blue-600 text-white text-xs px-2 py-1 rounded transition-all duration-200 transform hover:scale-105">
                                                        ✏️
                                                    </a>
                                                </div>
                                            </div>
                            ...
                            @foreach($calendar as $weekIndex => $week)
                                @foreach($week as $dayIndex => $day)
                                    <div class="p-px" style="width: 14.2857%;">
                                        @if($day['is_current_month'])
                                            <a href="{{ route('salon.schedules.create', ['master_id' => $selectedMaster->id, 'date' => $day['date']->format('Y-m-d')]) }}"
                                               class="block relative rounded-sm transition-all duration-200 {{ $day['is_today'] ? 'border-2 border-blue-500 bg-blue-50' : 'border border-gray-200' }} bg-white hover:bg-gray-50 hover:shadow-md active:bg-gray-100"
                                               style="aspect-ratio: 1; min-height: 48px;">
                                                <!-- Day Number -->
                                                <div class="p-1 h-full flex flex-col">
                                                    <div class="flex items-start space-x-1">
                                                        @if($day['is_today'])
                                                            <span class="text-xs font-medium text-blue-600">{{ $day['date']->day }}</span>
                                                        @else
                                                            <span class="text-xs font-medium text-gray-900">{{ $day['date']->day }}</span>
                                                        @endif
                                                        @if(!$day['has_schedule'])
                                                            <span class="w-3 h-3 bg-blue-500 text-white rounded-full flex items-center justify-center opacity-60">
                                        ...
                                        @else
                                            <div class="relative border border-gray-200 rounded-sm bg-gray-50" 
                                                 style="aspect-ratio: 1; min-height: 48px;">
                                                <div class="p-1 h-full flex flex-col">
